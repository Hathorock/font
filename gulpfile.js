...
var fontName = 'hathorock';    // set name of your symbol font
const className = 'h'        // set class name in your CSS
...
// Rename files with for 16bits digits
gulp.task('renameCode', function(){
  return gulp.src(['svg/**/u*.svg'])
    .pipe(vinylPaths(del)) // delete the original disk copy
    .pipe(rename(function (path) {
      const code = ('0000' + path.basename.replace(/^u([^-]*)-.*/,'$1')).substr(-4);
      const name = path.basename.replace(/^u([^-]*)-/,'')
      path.basename = 'u'+code+'-'+name;
    }))
    .pipe(gulp.dest("./svg"));
});
...
gulp.task('store', function(){
  return gulp.src(['svg/**/u*.svg'])
    .pipe(rename(function (path) {
      path.dirname = '';
      path.basename = className + '-' + path.basename.replace(/u([^-]*)-/,'');
    }))
    .pipe(svgmin({
      plugins: [{
...
gulp.task("default", gulp.series(gulp.task("Iconfont"), gulp.task("store"), gulp.task("renameCSS"), gulp.task("renameCode")));
